...
function calculator() {
  ...
  if (localStorage.getItem('sex')) {
    // если значение localStorage усиановлено
    sex = localStorage.getItem('sex'); // то присваиваем его переменной sex
  } else {
    // если значение localStorage не установлено
  ...
  function calcTotal() {
    // основная функция для подсчета каллорий
    if (!sex || !height || !weight || !age || !ratio) {
      // функция отрабатывает только при заполнении всех данных
  ...
  function initLocalSettings(selector, activeClass) {
    ...
    elements.forEach(elem => {
      ...
      if (elem.getAttribute('id') === localStorage.getItem('sex')) {
        // проверяем id и сравниваем с localStorage
    ...
    elements.forEach(elem => {
      elem.addEventListener('click', e => {
        ...
        if (e.target.getAttribute('data-ratio')) {
          // если у блока есть data-атрибут, то меняем data-ratio
          ratio = +e.target.getAttribute('data-ratio');
          localStorage.setItem('ratio', +e.target.getAttribute('data-ratio'));
        } else {
          // или если у блока есть id, то меняем его
          sex = e.target.getAttribute('id');
          localStorage.setItem('sex', e.target.getAttribute('id'));
        }

        elements.forEach(elem => {
          // сначала activeClass убираем у всех элементов
  ...
  function getDynamicInformation(selector) {
    ...
    input.addEventListener('input', () => {
      if (input.value.match(/\D/g)) {
        // если пользователем введено не число
      ...
      switch (input.getAttribute('id')) {
        // при помощи swith case проверяем в какой input кликнул пользователь
        case "height":
          height = +input.value;
          break;
...
/* harmony default export */ __webpack_exports__["default"] = (calculator); // экспортируем функцию
...
function cards() {
  class MenuCard {
    ...
    changeToUAH() {
      // метод для конвертации гривны в доллар
    ...
    render() {
      ...
      if (this.classes.length === 0) {
        // условие ксли пользователь забудет прописать класс
        ...
        // если же есть хотя бы 1 класс, то запускаем forEach, добавляющий этот класс во все карточки
  ...
  .then(data => {
    data.forEach(_ref => {
      ...
      // объект деструктуризируем
      new MenuCard(img, altimg, title, descr, price, '.menu .container').render(); // создаем новую карточку на странице и ее рендерим
    });
  });
}
...
function forms(formSelector, modalTimerId) {
  ...
  forms.forEach(item => {
    // Самое главное взять все наши формы и под каждую подвязать функцию postData
    bindPostData(item);
  });
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      ...
      const json = JSON.stringify(Object.fromEntries(formData.entries())); // formData превращаем в массив массивов, потом в объект, который превращаем в JSON
...
/* harmony default export */ __webpack_exports__["default"] = (forms); // экспортируем функцию
...
function openModal(modalSelector, modalTimerId) {
  ...
  if (modalTimerId) {
    // если modalTimerId существует, то запускаем 
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  modalTrigger.forEach(btn => {
    // перебираем псевдомассив для всех кнопок btn
    btn.addEventListener('click', () => openModal(modalSelector, modalTimerId));
  });
  modal.addEventListener('click', e => {
    // Закрытие модального окна по клику в любую точку или на кнопку Esc
    if (e.target === modal || e.target.getAttribute('data-close') == '') {
      //если клик по подложке или х, то закрываем
      closeModal(modalSelector); // closeModal вызываем
    }
  });
  document.addEventListener('keydown', e => {
    // При нажатии на Esc вызывается closeModal закрывающая модальное окно
  ...
  function showModalByScrol() {
    if (window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight - 1) {
      ...
      window.removeEventListener('scroll', showModalByScrol); // после появления modal удаляем showModalByScrol чтоб modal больше не появлялось при прокрутке до конца
    }
  }
...
/* harmony default export */ __webpack_exports__["default"] = (modal); // экспортируем функцию по умолчанию
...
function slider(_ref) {
  ...
  function plusZero(slides) {
    // функция для замены повторяющегося выражения
    if (slides.length < 10) {
      // если количество слайдов меньше 10,
  ...
  if (slides.length < 10) {
    ...
    // если количество слайдов больше 10, 
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    if (i == 0) {
      // добавляем класс активности первой точке
      dot.style.opacity = 1;
    }
  ...
  next.addEventListener('click', () => {
    if (offset == deleteNotDigits(width) * (slides.length - 1)) {
      ...
      // если не последний слайд, то добавляем смещение
  ...
  prev.addEventListener('click', () => {
    if (offset == 0) {
      // если доходим до первого слайда, тогда возвращаемся на последний
      offset = deleteNotDigits(width) * (slides.length - 1);
    } else {
      // если не первый слайд, то добавляем смещение
      offset -= deleteNotDigits(width);
    }
...
/* harmony default export */ __webpack_exports__["default"] = (slider); // экспортируем функцию
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  tabsParent.addEventListener('click', event => {
    ...
    if (target && target.classList.contains(tabsSelector.slice(1))) {
      // проверка что мы действительно кликаем в табы
      tabs.forEach((item, i) => {
        // item - каждый таб, i - номер таба по порядку
        if (target == item) {
          hideTabContent();
          showTabContent(i);
        }
...
/* harmony default export */ __webpack_exports__["default"] = (tabs); // экспортируем функцию
...
function timer(id, deadLine) {
  function getTimeRemaining(endtime) {
    // получаем разницу между дедлайн и текущей датой в милисекундах
  ...
  function getZero(num) {
    // Подставим ноль в показания дней и часов если там одинарное число
    if (num >= 0 && num < 10) {
      return `0${num}`;
    } else {
      return num;
  ...
  function setClock(selector, endtime) {
    ...
    function updateClock() {
      ...
      if (t.total <= 0) {
        // если время вышло <= 0, то мы больше таймер не обновляем
...
window.addEventListener('DOMContentLoaded', () => {
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_5__.openModal)('.modal', modalTimerId), 50000); // вызов modal через 5 сек
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_7__["default"])('.timer', '2023-01-01');
});
}();
/******/ })()
;
